<?php
/*
Plugin Name: SEO Slugs for bbPress
Plugin URI: http://amemiku.tumblr.com/post/16267587665/seo-slugs-bbpress-plugin
Description: Removes common words like 'a', 'the', 'in' from post slugs to improve SEO.
Version: 1.0
Author: Amemiku
Author URI: http://exit4rum.com/
*/
...
add_filter('wp_insert_post_data','seo_slugs_for_bbp',10,2);

function seo_slugs_for_bbp($data, $postarr) {
	$data['post_name'] = seo_slugs($data['post_name']);
	return $data;
}

function seo_slugs($post_name) {

	$seo_slug = strtolower(stripslashes($post_name));
